...
class Environment():
    ...
    def displayMessage(self, path):
        
        pg.font.init()
        myfont = pg.font.SysFont('Comic Sans MS', 32, True)
        myfont2 = pg.font.SysFont('Comic Sans MS', 16, True)

        if(len(path) >= 20):
            self.screen.fill((0, 0, 0))
            displaytxt = myfont.render('Sorry! Connot find path' , True, (255, 255, 255))
            self.screen.blit(displaytxt, (5, 50))
        else: 
            self.screen.fill((0, 255, 0))
            displaytxt = myfont.render('Goal has been reached!!!' , True, (255, 255, 255))
            pathtxt = myfont2.render('Path:' + str(path), True, (255, 255, 255))
            self.screen.blit(displaytxt, (5, 50))
            self.screen.blit(pathtxt, (5, 100))
        pg.display.flip()
        pg.time.wait(5000)
